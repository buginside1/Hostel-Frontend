  ...
  const submitHandler = async () => {
    ...
    try {
      const { data } = await axios.post(
        ...
        { amount: bookingDetails.totalPricePerDay },
        ...
        <div className="px-1 sm:px-3 flex justify-center">
          <div className="flex flex-col gap-4">
            ...
            <button
              ...
              type="submit"
              ref={payBtn}
              className="py-4 w-56 block text-center rounded bg-blue-400 hover:bg-blue-500 transition duration-200 text-zinc-50 disabled:cursor-not-allowed"
            >{`Pay - ${bookingDetails?.totalPricePerDay}`}</button>
          </div>
        </div>
      </div>
    </Fragment>
