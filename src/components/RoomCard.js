    ...
    <div className="flex flex-col w-80 gap-8 bg-stone-100 shadow-md rounded-md">
      ...
      <div className="mx-4 mb-6">
        ...
        <p className=" my-3 font-medium">
          Room Type: <span className="font-normal font-mono">{room.type}</span>
        </p>
        ...
        <span className="text-xl font-medium">
          <RequestQuoteIcon className="mb-1 mr-1" />
          {room.PricePerDay} Rs
        </span>
        <div className="flex justify-between gap-4 mt-4">
          {/* <button
            onClick={() => setIsDateOpen(!isDateOpen)}
            className=" border-blue-400 text-blue-400 hover:text-blue-500 hover:border-blue-500 hover:bg-blue-200 border-solid border py-2 rounded-lg w-36 text-center transition duration-200 box-border"
          >
            Availability
          </button> */}
          <button
            onClick={() => navigate(`/room/${room._id}/book`)}
            className="bg-blue-500 hover:bg-400 py-2 rounded-lg w-36 text-center text-neutral-50  transition duration-200 font-semibold"
          >
            Reserve Seat
          </button>
        </div>
